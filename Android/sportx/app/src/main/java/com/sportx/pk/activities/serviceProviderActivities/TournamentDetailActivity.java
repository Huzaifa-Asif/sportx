...
import com.sportx.pk.Adapters.TournamentTeamsAdapter;
// for completed tournament we use customer teams adapter
import com.sportx.pk.Adapters.CustomerTournamentTeamsAdapter;
...
public class TournamentDetailActivity extends Menu implements View.OnClickListener {
    ...
    // for completed tournament we use customer teams adapter
    private CustomerTournamentTeamsAdapter CustomerTeamAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        super.inflateView(R.layout.activity_sh_tournament_detail);
        setTitle("Tournament");
...
//        create_fixtures.setOnClickListener(this);
        ...
        if(state.equals("completed")){
            // for completed tournament we use customer teams adapter
            CustomerTeamAdapter = new CustomerTournamentTeamsAdapter(this, teams);
            teamRecycler.setAdapter(CustomerTeamAdapter);
        }
        ...
        // IF Tournament is completed then Tournament state change button, cancel button and add team button
        // Should disappear
        if(state.equals("completed")){
            tournament_state_.setVisibility(View.GONE);
            cancel_tournament.setVisibility(View.GONE);
            add_team.hide();
        }
                    ...
                    public void onCompleted(Exception e, Response<String> result) {
                        ...
                        try{
                            ...
                            if (!status) {
                                ...
                                if(teams.size()!=0){
                                    _team_text.setVisibility(View.GONE);
                                }
                            }
...
//                                Log.d( "TeamPlayers: ",TeamPlayers.getString(0));
                            ...
                            // because the adapter is different in inactive and active tournament
                            if (state.equals("active") || state.equals("inactive")) {
                                teamAdapter.notifyDataSetChanged();
                            }

                            // the adapter is different in completed tournament
                            if (state.equals("completed")) {
                                CustomerTeamAdapter.notifyDataSetChanged();
                            }
