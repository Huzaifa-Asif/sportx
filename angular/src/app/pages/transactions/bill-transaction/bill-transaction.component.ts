...
export class BillTransactionComponent implements OnInit {
  ...
  isSptFormFilled = false;

  constructor(private api: RestApiService, private auth: AuthService,
    private router: Router, private helper: HelperService, private modalService: NgbModal) {
  ...
  getData() {
    this.api.get('Transactions/GetData').then((data: any) => {
  ...
  submit() {
    ...
    if (this.validation.minMax && this.validation.stock && this.validation.sptForm) {
  ...
  _check() {
    ...
    // SPT Form:
    if (this.suspiciousTransaction && !this.isSptFormFilled) {
      this.validation.sptForm = false;
      this.validation.sptError = 'Kindly Fill Suspicious Transaction Form';
      this._openSPFModal();
    } else {
      this.validation.sptForm = true;
      this.validation.sptError = '';
    }
  }

  _sendSaveRequest(data) {
    this.api.post('Transactions/SaveBillTransaction', data).then((response: any) => {
  ...
  _openSPFModal() {
    const modalRef = this.modalService.open(SuspiciousTransactionComponent, { size: 'lg', backdrop: 'static' });

    modalRef.componentInstance.siName = this.customer.name;
    modalRef.componentInstance.siAddress = this.customer.address;
    modalRef.componentInstance.siCellNo = this.customer.contactNo;
    modalRef.componentInstance.siCNIC = this.documentNumber;
    modalRef.componentInstance.stiAmount = this.total;

    modalRef.result.then((added) => {

      if (added) {
        this.isSptFormFilled = true;
      }
